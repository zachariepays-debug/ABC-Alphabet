...
# --- 2. ÉCRAN DE CHARGEMENT (5 SECONDES) ---
if 'chargement_fini' not in st.session_state:
    ...
    with placeholder.container():
        st.markdown("""
            ...
            .ballon { font-size: 100px; animation: bounce 1s infinite alternate; }
            @keyframes bounce { from { transform: translateY(0); } to { transform: translateY(-30px); } }
            .txt-load { color: #8A63FF; font-size: 30px; margin-top: 20px; }
            </style>
            <div class="loading-container">
                <div class="ballon">🎈</div>
                <div class="txt-load">Préparation de la magie...</div>
            </div>
...
# --- 4. LOGIQUE IA (DOUDOU & DÉFINITIONS) ---
def ia_magique(prompt, mode="doudou"):
    if not MISTRAL_API_KEY:
        return "Branche ma clé magique !"
    system_instruction = "Tu es un doudou gentil pour enfant de 3 ans. Si mode=dico: Donne UNIQUEMENT la définition simple. Si mode=doudou: Sois joyeux et court."
    url = "https://api.mistral.ai/v1/chat/completions"
    headers = {"Authorization": f"Bearer {MISTRAL_API_KEY}"}
    data = {
        "model": "mistral-tiny",
        "messages": [{"role": "system", "content": system_instruction}, {"role": "user", "content": f"Mode={mode}. {prompt}"}]
    }
    try:
...
st.markdown(f"""
    ...
    .titre-enfant, label, p, .stMarkdown, .stTextInput label {{ color: #2C3E50 !important; font-family: 'Fredoka One', cursive !important; }}
    .titre-enfant {{ text-align: center; font-size: 42px !important; color: #8A63FF !important; text-shadow: 2px 2px 0px #FFFFFF; }}
    input {{ color: #000000 !important; background-color: #FFFFFF !important; border: 4px solid !important; border-image: linear-gradient(to right, #FF1493, #00BFFF, #00FF7F, #FFD700) 1 !important; border-radius: 15px !important; font-size: 22px !important; }}
    .stButton > button {{ background: #FFFFFF !important; border: 4px solid #8A63FF !important; border-radius: 20px !important; color: #8A63FF !important; font-family: 'Fredoka One', cursive !important; font-size: 20px !important; height: 70px !important; width: 100% !important; box-shadow: 0px 4px 0px #8A63FF !important; }}
...
if st.session_state.mode == "calc":
    ...
    if st.button("🌟 EXPLIQUE-MOI LE MOT"):
        if mot_saisi:
            res = ia_magique(mot_saisi, mode="dico")
            st.success(res); parler(res)
    ...
    if st.button("PARLER AU DOUDOU"):
        res = ia_magique(q, mode="doudou")
        st.info(res); parler(res)

else:
    # Système de dossiers classique
    cols = st.columns(3)
